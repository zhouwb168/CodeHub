$(function () {
    ...
    grid.datagrid("getColumnOption", "Greater").formatter = Events.ToText;
...
var Events = {
    ...
    ToText: function (value, row, rowIndex) {
        var strValue = "";
        if (row.Bad != null) {
            strValue += (strValue == "" ? "腐朽: " : "<br />腐朽: ") + row.Bad + "%";
        }
        if (row.Greater != null) {
            strValue += (strValue == "" ? "过大: " : "<br />过大: ") + row.Greater + "%";
        }
        if (row.Less != null) {
            strValue += (strValue == "" ? "过小: " : "<br />过小: ") + row.Less + "%";
        }
    ...
    GetGrid: function (callback) {
        ...
        Eventer.Grid(grid, Events.Service, 'GetReport01ByDateAndStartAndLength', [startDate, endDate, rowStart, pageSize], callback);
    },

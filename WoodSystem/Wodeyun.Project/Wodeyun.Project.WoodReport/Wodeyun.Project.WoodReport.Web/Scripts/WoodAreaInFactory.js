        ...
        var callback = function () {
            ...
            for (var i = 0; i < rowCount; i++) {
                ...
                if (currentGroup == prevGroup) {
                    sheet.Range('A' + (currentRowIndex - 1) + ':' + 'A' + currentRowIndex).MergeCells = true;
                    sheet.Range('D' + (currentRowIndex - 1) + ':' + 'D' + currentRowIndex).MergeCells = true;
                    sheet.Range('F' + (currentRowIndex - 1) + ':' + 'F' + currentRowIndex).MergeCells = true;
                }
            ...
            SetValueAndStyle(sheet.Cells(4 + rowCount + 1, 6), rows[rowCount], columns[5]);
            sheet.Range('A4:' + 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.substring(columnCount - 1, columnCount) + (currentRowIndex + 1)).Borders.Weight = 2; // 设置单元格边框
        };

$(function () {
    ...
    grid.datagrid("getColumnOption", "MaxWater").formatter = Events.ToFixed;
    grid.datagrid("getColumnOption", "MinWater").formatter = Events.ToFixed;
    grid.datagrid("getColumnOption", "TotalAvgWater").formatter = Events.ToFixed;
...
var Events = {
    ...
    GetGrid: function (callback) {
        ...
        Eventer.Grid(grid, Events.Service, 'GetReport06ByMonthAndStartAndLength', [month, rowStart, pageSize], callback);
    },
    ...
    */
    mergeCellsByField: function () {
        var tableID = "Grid", colList = "Area,TotalAvgWater";
    ...
    OnExcel: function () {
        ...
        var callback = function () {
            ...
            for (var i = 0; i < rowCount; i++) {
                currentRowIndex += 1;
                currentGroup = rows[i].Area; // 获取当前记录的组标识
                if (currentGroup == prevGroup) {
                    sheet.Range('B' + (currentRowIndex - 1) + ':' + 'B' + currentRowIndex).MergeCells = true;
                    sheet.Range('I' + (currentRowIndex - 1) + ':' + 'I' + currentRowIndex).MergeCells = true;
                }

$(function () {
    ...
    grid.datagrid("getColumnOption", "CheckTime").formatter = Events.ToDateTime;
    grid.datagrid("getColumnOption", "LaboratoryPeople").formatter = Events.ToText;
    grid.datagrid("getColumnOption", "IsConfirmed").formatter = Events.ToChange;
    grid.datagrid("getColumnOption", "ConfirmePeople").formatter = Events.ToText;
...
var Events = {
    ...
    ToChange: function (value) {
        if (value == null) return '';

        var htmlValue = "";
        if (value == false) htmlValue = "<span style=\"color:red;\">否</span>";
        else htmlValue = "是";
    ...
    GetGrid: function (callback) {
        ...
        Eventer.Grid(grid, Events.Service, 'GetReport04BySearchWithPaging', [startDate, endDate, rowStart, pageSize, supplier, license, key], callback);
    },
    ...
    OnExcel: function () {
        ...
        var callback = function () {
            var SetValueAndStyle = function (cell, row, column) {
                ...
                if (column == 'CheckTime') result = Events.ToDateTime(result);
                if (column == 'LaboratoryPeople') result = Events.ToText(result);
                if (column == 'IsConfirmed') result = result == false ? "否" : "是";
                if (column == 'ConfirmePeople') result = Events.ToText(result);

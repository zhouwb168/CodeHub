$(function () {
    ...
    grid.datagrid("getColumnOption", "PeopleOfFullPound").formatter = Events.ToText;
    grid.datagrid("getColumnOption", "BackWeighTime").formatter = Events.ToDateTime;
    grid.datagrid("getColumnOption", "PeopleOfEmptyPound").formatter = Events.ToText;
...
var Events = {
    ...
    compute: function () {//计算函数
        ...
        var success = function (result) {
            ...
            if (root != null) {
                $("#datahtml").html("&nbsp;汇总：首磅体积：<span style='color:red'>" + (root.SUMFullVolume == null ? 0 : root.SUMFullVolume).toFixed(2) + "</span>&nbsp;&nbsp;回皮体积：<span style='color:red'>" + (root.SUMEmptyVolume == null ? 0 : root.SUMEmptyVolume).toFixed(2) + "</span>&nbsp;&nbsp;扣方：<span style='color:red'>" + (root.SUMRebateVolume == null ? 0 : root.SUMRebateVolume).toFixed(2) + "</span>&nbsp;&nbsp;净体积：<span style='color:red'>" + (root.SUMDiscVolume == null ? 0 : root.SUMDiscVolume).toFixed(2) + "</span>");
            }
        };
    ...
    GetGrid: function (callback) {
        ...
        var footer = function () {
            ...
            var DiscVolume = 0.00, jWeight = 0.00, RebateVolume = 0.00
            , FullVolume = 0.00, EmptyVolume = 0.00, HandVolume = 0.00;

            for (var i = 0; i < items.length; i++) {
                FullVolume += items[i].FullVolume;
                DiscVolume += items[i].DiscVolume;
                jWeight += items[i].jWeight;
                EmptyVolume += items[i].EmptyVolume;
                HandVolume += items[i].HandVolume;
                RebateVolume += items[i].RebateVolume;
            }

            grid.datagrid('reloadFooter', [{
                License: '<font color="red"><b>合计</b></font>'
                , jWeight: '<font color="red"><b>' + jWeight.toFixed(2) + '</b></font>'
                , DiscVolume: '<font color="red"><b>' + DiscVolume.toFixed(2) + '</b></font>'
                , FullVolume: '<font color="red"><b>' + FullVolume.toFixed(2) + '</b></font>'
                , EmptyVolume: '<font color="red"><b>' + EmptyVolume.toFixed(2) + '</b></font>'
                , HandVolume: '<font color="red"><b>' + HandVolume.toFixed(2) + '</b></font>'
                , RebateVolume: '<font color="red"><b>' + RebateVolume.toFixed(2) + '</b></font>'
            }]);
            if (callback != null) callback();
        };

        Eventer.Grid(grid, Events.Service, 'GetReport02BySearchWithPaging', [startDate, endDate, rowStart, pageSize, supplier, license], footer);
    },
    ...
    OnExcel: function () {
        ...
        var callback = function () {
            var SetValueAndStyle = function (cell, row, column) {
                ...
                if (column == 'Bang_Time') result = Events.ToDateTime(result);
                if (column == 'PeopleOfFullPound') result = Events.ToText(result);
                if (column == 'BackWeighTime') result = Events.ToDateTime(result);
                if (column == 'PeopleOfEmptyPound') result = Events.ToText(result);

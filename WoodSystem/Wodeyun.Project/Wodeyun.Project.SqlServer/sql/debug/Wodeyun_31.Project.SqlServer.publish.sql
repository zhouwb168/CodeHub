...
:setvar DefaultDataPath "F:\Program Files\Microsoft SQL Server\MSSQL11.MSSERVER2012\MSSQL\DATA\"
:setvar DefaultLogPath "F:\Program Files\Microsoft SQL Server\MSSQL11.MSSERVER2012\MSSQL\DATA\"
...
IF EXISTS (select top 1 1 from [dbo].[CostStatement])
...
IF EXISTS (select top 1 1 from [dbo].[GsmJoin])
    RAISERROR (N'检测到行。由于可能丢失数据，正在终止架构更新。', 16, 127) WITH NOWAIT
...
IF EXISTS (select top 1 1 from [dbo].[WoodPrice])
    RAISERROR (N'检测到行。由于可能丢失数据，正在终止架构更新。', 16, 127) WITH NOWAIT
...
GO
PRINT N'正在删除 [dbo].[GsmItem].[IX_GsmItem]...';


GO
DROP INDEX [IX_GsmItem]
    ON [dbo].[GsmItem];


GO
PRINT N'正在删除 [dbo].[GsmMessage].[IX-Date]...';
...
GO
PRINT N'正在删除 [dbo].[WoodBang].[IX-Bang_Time]...';


GO
DROP INDEX [IX-Bang_Time]
    ON [dbo].[WoodBang];


GO
PRINT N'正在删除 [dbo].[CostStatement] 上未命名的约束...';


GO
ALTER TABLE [dbo].[CostStatement] DROP CONSTRAINT [DF__CostState__IsCon__336AA144];
...
GO
ALTER TABLE [dbo].[CostStatement] DROP CONSTRAINT [DF__CostState__FullV__3BFFE745];
...
GO
ALTER TABLE [dbo].[CostStatement] DROP CONSTRAINT [DF__CostState__CubeP__3CF40B7E];


GO
PRINT N'正在删除 [dbo].[WoodPrice] 上未命名的约束...';


GO
ALTER TABLE [dbo].[WoodPrice] DROP CONSTRAINT [DF__WoodPrice__CubeP__3B0BC30C];
...
GO
ALTER TABLE [dbo].[WoodPrice] DROP CONSTRAINT [DF__WoodPrice__WetPr__3A179ED3];


GO
PRINT N'正在删除 [dbo].[GsmJoin] 上未命名的约束...';


GO
ALTER TABLE [dbo].[GsmJoin] DROP CONSTRAINT [DF__GsmJoin__AreaID__52E34C9D];
...
GO
ALTER TABLE [dbo].[WoodPrice] DROP CONSTRAINT [DF__WoodPrice__IsCon__3FD07829];


GO
PRINT N'正在开始重新生成表 [dbo].[CostStatement]...';
...
CREATE TABLE [dbo].[tmp_ms_xx_CostStatement] (
    ...
    CONSTRAINT [tmp_ms_xx_constraint_PK_CostStatement_1] PRIMARY KEY CLUSTERED ([OrderNo] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[CostStatement])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_CostStatement] ([OrderNo], [Unique], [License], [LinkMan], [GWeight], [GPrice], [Amount], [State], [Operator], [OperatorDate], [Auditor], [IsPrint], [JWeight], [Area], [Tree], [GroupID], [Bang_Time], [Log], [IsConfirmed], [Confirmor], [ConfirmTime], [JVolume], [FullVolume], [CubePrice])
        SELECT   [OrderNo],
                 [Unique],
                 [License],
                 [LinkMan],
...
DROP TABLE [dbo].[CostStatement];

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_CostStatement]', N'CostStatement';

EXECUTE sp_rename N'[dbo].[tmp_ms_xx_constraint_PK_CostStatement_1]', N'PK_CostStatement_1', N'OBJECT';
...
GO
ALTER TABLE [dbo].[EmptyPound] DROP COLUMN [RebateVolume];


GO
PRINT N'正在改变 [dbo].[GsmJoin]...';


GO
ALTER TABLE [dbo].[GsmJoin] DROP COLUMN [AreaID];


GO
PRINT N'正在改变 [dbo].[WoodPrice]...';


GO
ALTER TABLE [dbo].[WoodPrice] DROP COLUMN [IsConfirmed];


GO
ALTER TABLE [dbo].[WoodPrice] ALTER COLUMN [CubePrice] DECIMAL (10, 2) NULL;

ALTER TABLE [dbo].[WoodPrice] ALTER COLUMN [WetPrice] DECIMAL (10, 2) NULL;
...
GO
PRINT N'正在创建 [dbo].[GsmMessage].[NonClusteredIndex-20141127-091628]...';
...
GO
PRINT N'正在创建 [dbo].[WoodBang].[NonClusteredIndex-Bang_Time]...';
...
GO
PRINT N'正在改变 [dbo].[F_SQLSERVER_SPLIT]...';

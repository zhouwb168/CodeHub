...
var Eventer = {
    ...
    Delete: function (unique, table, refresh, buttons, controls, record, operator, greenCardNumber, redCardNumber) {
        ...
        var data = {
            ...
            args: [parseInt(unique.val()), parseInt(record.val()), parseInt(operator), greenCardNumber, redCardNumber, controls.Get().BangCID]
        };
        var success = function (result) {
            var root = Ajaxer.GetRoot(result);
    ...
    Save: function (unique, table, refresh, buttons, controls, greenCardNumber, redCardNumber) {
        var entity = controls.Get();
        var data = {
            service: table,
            method: 'SaveEntityByOperator',
            args: [JSON.stringify(entity), greenCardNumber, redCardNumber]
        };
        var success = function (result) {
            var root = Ajaxer.GetRoot(result);
    ...
    //保存即打印
    OnNowPrint: function (entity) {
        if (!confirm("是否打印磅单？")) return;
        var emptyvolume = 0, jvolume = 0, rebatevolume = 0;
        emptyvolume = entity.EmptyVolume;
        rebatevolume = entity.RebateVolume;
        jvolume = entity.JVolume;
        ...
        txtBox.name = "params";
        txtBox.type = "text";
        txtBox.value = escape(emptyvolume + "|" + jvolume + "|" + rebatevolume);

...
var Eventer = {
    ...
    Delete: function (unique, table, refresh, buttons, controls, operator, greenCardNumber) {
        if (confirm("是否确定要删除该记录？") == false) return;

        var data = {
            service: table,
            method: 'DeleteEntityByUniqueWithOperator',
            args: [parseInt(unique.val()), parseInt(operator), greenCardNumber]
        };
        var success = function (result) {
            var root = Ajaxer.GetRoot(result);
    ...
    Save: function (unique, table, refresh, buttons, controls) {
        ...
        var success = function (result) {
            ...
            controls.PermitPhoto(root.Value);

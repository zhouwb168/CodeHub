$(function () {
    ...
    Events.GetGrid(moment().format('YYYY-MM'), 1, $('#Grid').treegrid('options').pageSize);
});
...
var Events = {
    ...
    ToNo: function (value, row) {
        if (value == null) return '<font color="red">' + row.Supplier + '</font>';
    ...
    GetGrid: function (month, start, length, callback) {
        ...
        Eventer.Grid($('#Grid'), Events.Service, 'GetGridByMethodAndFieldsAndUnique', ['GetReport04ByMonthAndStartAndLength', [month, start, length], ['Supplier'], 'Number'], footer);
    ...
    OnClick: function (event) {
        Events.GetGrid(moment().add('months', -event.data.index).format('YYYY-MM'), 1, $('#Grid').treegrid('options').pageSize);
    },

    OnQuery: function () {
        Events.GetGrid(Eventer.Get($('#Year')) + '-' + Eventer.Get($('#Month')), 1, $('#Grid').treegrid('options').pageSize);
    },
    ...
    OnExcel: function () {
        ...
        Events.GetGrid(Events.GetMonth(), 1, $('#Grid').treegrid('options').pageSize, callback);
    },
